...
- 证书完整性必须由具体的有限轨迹集合或迭代器实际耗尽来证明。候选提供的 `scenario_count()` 只作诊断提示，不能证明覆盖完整；部分覆盖即使已检查要求全部通过，也不能成为满足证书。
...
- `SatisfactionEvaluator`：惰性消费模拟轨迹，不信任候选自报的场景数，逐项检查要求并生成完整性、覆盖率、鲁棒余量和假设可靠性证书；同一轨迹批次可被多个规格安全复用。
...
- 双向往返检查：宏观往返比较规格语义，微观往返按“初始场景 + 干预”对齐任务行为，而不要求文字或内部结构相同；往返的全部阶段共用一份预算。
...
- 可执行模型的 `simulate`、资源指标和可观测读出；可选的 `scenario_count` 仅提供诊断信息；
...
- 对未知长度的惰性迭代器，恰好触及模拟上限时无法在不多取一个场景的前提下证明已经耗尽，因此证书会保守地标记为不完整；若领域能够在内存中安全枚举有限场景，可返回普通 `tuple` 或 `list` 来提供可验证的总数。
